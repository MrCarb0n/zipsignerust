/*
 * ZipSigner Rust v3.0.0 (Professional Edition)
 * --------------------------------------------
 * A high-performance, memory-safe tool for signing Android OTA packages.
...
// --- Brand Constants ---
const APP_NAME: &str = "ZipSigner";
const APP_BIN_NAME: &str = "zipsignerust";
const APP_VERSION: &str = "3.0.0";
const APP_AUTHOR: &str = "Tiash H Kabir (@MrCarb0n)";
const APP_ABOUT: &str = "High-performance cryptographic signer for Android ZIP/OTA packages.";
...
pub fn build_command() -> Command {
    Command::new(APP_NAME)
        ...
        .long_about(format!(
            "{}\n\n{}",
            APP_ABOUT,
            "Features:\n  • Streaming I/O for low memory usage\n  • Strict timestamp inheritance for reproducible builds\n  • OpenSSL backend for security"
        ))
        .after_help(
            format!("{}",
            "EXAMPLES:\n  \
             zipsignerust input.zip output.zip       # Sign a file\n  \
             zipsignerust --verify signed.zip        # Verify a signature\n  \
             zipsignerust -i input.zip               # Sign in-place (overwrite)\n\n\
             keys generated by @MrCarb0n".bright_black())
        )
        ...
        .arg(Arg::new("verify").short('v').long("verify").action(ArgAction::SetTrue).help("Verify digital signature integrity"))
        .arg(Arg::new("private_key").short('k').long("private-key").help("Custom private key (PEM)"))
        .arg(Arg::new("public_key").short('p').long("public-key").help("Custom public key/cert (PEM)"))
        .arg(Arg::new("overwrite").short('f').long("overwrite").action(ArgAction::SetTrue).help("Force overwrite if output exists"))
        .arg(Arg::new("inplace").short('i').long("inplace").action(ArgAction::SetTrue).help("Sign input file directly (creates backup)"))
}

pub fn main() {
    let matches = build_command().get_matches();

    // Print Banner on startup (unless verifying)
...
// --- Logic ---
pub fn run(matches: &clap::ArgMatches) -> Result<(), SignerError> {
    ...
    fn ensure_certificate(&mut self) -> Result<(), SignerError> {
        if self.certificate.is_none() {
            if let (Some(pk), Some(pubk)) = (&self.private_key, &self.public_key) {
                ...
                name_builder.append_entry_by_text("CN", "ZipSigner")?;
...
impl ArtifactProcessor {
    ...
    fn gen_sf(manifest: &[u8]) -> Vec<u8> {
        let mut out = format!("Signature-Version: 1.0\r\nCreated-By: {}\r\nSHA1-Digest-Manifest: {}\r\n\r\n",
            APP_NAME, CryptoEngine::compute_sha1(manifest)).into_bytes();
